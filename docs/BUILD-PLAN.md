...
3. **Output dir layout.** Should the generator emit `out/recipes/<overlay>.json` (matching the lynx docker_files layout) or `out/<recipe>.json` at root (matching the lynx in-container layout)? They're consistent in different ways. Lean: match the in-container layout (recipes at root) since that's the runtime contract; the docker_files-with-`recipes/` shape was a pre-fix-#a7a2497 artifact.
